...
router.get("/", function (req, res, next) {
  ...
  if (search && startIndex) {
    ...
    google_images({ query: search, nums: srchNum, size: quality, pagination: startLookingAtIndex, safe: secure, imageType:typeOfImage }, actualPage)
      ...
      .catch((err) => res.render("error", { error: { status: 500 }, message: "Sorry, we're having trouble searching, try searching differently or using keywords. Perhaps the result was not found or no longer has related content Or it could also be that the google API unfortunately hit the daily search limit (considering it's free)" }));
